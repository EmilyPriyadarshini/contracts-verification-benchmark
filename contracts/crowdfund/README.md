...
- **v2**: donation period never ends, i.e end_donate = type(uint256).max and no `require (block.number > end_donate)` check in `withdraw`.
- **v3**: `owner` not immutable and `setOwner` allows any user to set itself as owner.
